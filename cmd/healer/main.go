// Command healer detects a misconfiguration in the cluster, diagnoses it and opens
// a pull request on ft_lgtm_gitops that repairs it.
//
...
func main() {
	if err := watch(); err != nil {
		fmt.Fprintf(os.Stderr, "lgtm-healer: %v\n", err)
		os.Exit(1)
	}
}
...
func watch() error {
	...
	if err := flush(closing); err != nil {
		fmt.Fprintf(os.Stderr, "lgtm-healer: the telemetry did not flush: %v\n", err)
	}
	return nil
}
